...
class Airtable():
    ...
    def get(self, table_name, record_id=None, limit=0, offset=None, sort_field=None, sort_asc=True):
        params = {}
        if check_string(record_id):
            ...
            if check_string(sort_field):
                params.update({'sortField': sort_field})
            if not sort_asc:
                params.update({'sortDirection': 'desc'})
